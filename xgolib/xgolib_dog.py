...
XGOorder = {
    ...
    "WHEEL_SPEED": [0xB1, 0x80, 0x80, 0x80, 0x80],
    "WHEEL_ENCODER": [0xB2, 0x80, 0x80, 0x80, 0x80],
}
...
class XGO_DOG():
    ...
    def __init__(self, port="/dev/ttyAMA0", baud=115200, version="xgomini", verbose=False):
        ...
        if self.version[0] == 'M':
            ...
            changePara('xgomini')
            print("This is a XGO-RIDER device")
        else:
            #changePara('xgorider')
            changePara('xgomini')
    ...
    def __send_byte(self, addr, value):
        tx = [0x55, 0x00, 0x09, 0x00, addr, value]
        sum_data = (0x09 + 0x00 + addr + value) % 256
    ...
    def arm_rotate(self, angle):
        value = conver2u8(angle, [-175, 175])
        self.__send_byte(0x70, value)

    def arm_motor(self, angles):
        value = conver2u8(angles[0], [-50, 83])
        self.__send_byte(0x5D, value)
        time.sleep(0.001)
        value = conver2u8(angles[1], [-90, 90])
        self.__send_byte(0x5E, value)
        time.sleep(0.001)
        value = conver2u8(angles[2], [-175, 175])
        self.__send_byte(0x70, value)
        time.sleep(0.001)
...
#####MINI3W####
    ...
    def wheel_control(self, data):
        for i in range(4):
            XGOorder["WHEEL_SPEED"][i + 1] = data[i]
            self.__send("WHEEL_SPEED", len=4)

    def wheel_get_all_encoder(self):
        self.__read(XGOorder["WHEEL_ENCODER"][0], 16)
        encoders = [0] * 4
        if self.__unpack():
            encoders = struct.unpack('<4i', bytes(self.rx_data[0:16]))
        return encoders

    def wheel_get_encoder(self, id):
        return self.wheel_get_all_encoder()[id - 1]

    def wheel_speed(self, speed):
        data = [0x80] * 4
        for i in range(4):
            data[i] = conver2u8(speed[i], 1.5)
        print(data)
        self.wheel_control(data)

    def wheel_get_all_speed(self):
        self.__read(XGOorder["WHEEL_SPEED"][0], 16)
        speeds = [0] * 4
        if self.__unpack():
            values = struct.unpack('<4f', bytes(self.rx_data[0:16]))
            speeds = [round(v, 5) for v in values]
        return speeds

    def wheel_get_speed(self, id):
        return self.wheel_get_all_speed()[id - 1]

    def wheel_get_circle_pulse(self):
